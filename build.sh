...
show_help() {
    ...
    echo "  --kafl              Set fuzzer to kafl (otherwise libfuzzer)"
...
then
    HOST_COMPILE_FLAGS+=" -fsanitize-coverage=inline-8bit-counters,bb,no-prune,pc-table,trace-cmp"
...
rm ${PROJ_DIR}/install
ln -s ${INSTALL_DIR} ${PROJ_DIR}/install

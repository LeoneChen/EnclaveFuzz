...
OPTS=$(getopt -o ht:gbs -l help,target:,debug,build,setu,sgx -n 'parse-options' -- "$@")
eval set -- "$OPTS"
while true; do
    case "$1" in
        -h|--help)
    ...
    case "${TARGET_NAME}" in
        ...
        "wasm-micro-runtime")
            pushd ${TARGET_DIR}
                ...
                pushd product-mini/platforms/linux-sgx
                    rm -rf build
                    if [ ${AS_SGX} -eq 1 ]; then
                        cmake -B build ${DEBUG_CMAKE_FLAGS} -DWAMR_BUILD_SIMD=0
                    else
                    ...
                    pushd enclave-sample
                        make clean
                        if [ ${AS_SGX} -eq 1 ]; then
                            make SGX_MODE=SIM ${DEBUG_MAKE_FLAGS} -j${JOBS}
                        else
        ...
        "sgxwallet")
            pushd ${TARGET_DIR}
                ...
                if [ ${AS_SGX} -eq 1 ]; then
                    ./configure --enable-sgx-simulation CFLAGS="${DEBUG_FLAG}" CXXFLAGS="${DEBUG_FLAG}"
                else

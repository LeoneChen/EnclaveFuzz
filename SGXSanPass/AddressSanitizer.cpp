...
static cl::opt<bool>
    ClRecover("sgxsan-recover",
              cl::desc("Enable recovery mode (continue-after-error)."),
              cl::Hidden, cl::init(false));

static cl::opt<bool> ClInsertVersionCheck(
    "sgxsan-guard-against-version-mismatch",
    cl::desc("Guard against compiler/runtime version mismatch."), cl::Hidden,
    cl::init(false));

// This flag may need to be replaced with -f[no-]asan-reads.
static cl::opt<bool> ClInstrumentReads("sgxsan-instrument-reads",
                                       cl::desc("instrument read instructions"),
                                       cl::Hidden, cl::init(true));

static cl::opt<bool>
    ClInstrumentWrites("sgxsan-instrument-writes",
...
static cl::opt<bool>
    ClInstrumentByval("sgxsan-instrument-byval",
...
static cl::opt<bool> ClForceDynamicShadow(
    "sgxsan-force-dynamic-shadow",
    cl::desc("Load shadow address into a local variable for each function"),
    cl::Hidden, cl::init(false));

static cl::opt<bool>
    ClWithIfunc("sgxsan-with-ifunc",
                cl::desc("Access dynamic shadow through an ifunc global on "
                         "platforms that support this"),
                cl::Hidden, cl::init(true));

static cl::opt<bool> ClWithIfuncSuppressRemat(
    "sgxsan-with-ifunc-suppress-remat",
...
static cl::opt<bool> ClStack("sgxsan-stack", cl::desc("Handle stack memory"),
                             cl::Hidden, cl::init(true));
...
static cl::opt<bool> ClRedzoneByvalArgs("sgxsan-redzone-byval-args",
                                        cl::desc("Create redzones for byval "
                                                 "arguments (extra copy "
                                                 "required)"),
                                        cl::Hidden, cl::init(true));

static cl::opt<bool> ClUseAfterScope("sgxsan-use-after-scope",
...
static cl::opt<bool> ClInitializers("sgxsan-initialization-order",
                                    cl::desc("Handle C++ initializer order"),
                                    cl::Hidden, cl::init(false));

static cl::opt<bool> ClInvalidPointerPairs(
    "sgxsan-detect-invalid-pointer-pair",
    cl::desc("Instrument <, <=, >, >=, - with pointer operands"), cl::Hidden,
    cl::init(false));

static cl::opt<bool> ClInvalidPointerCmp(
    "sgxsan-detect-invalid-pointer-cmp",
    cl::desc("Instrument <, <=, >, >= with pointer operands"), cl::Hidden,
    cl::init(false));

static cl::opt<bool> ClInvalidPointerSub(
    "sgxsan-detect-invalid-pointer-sub",
...
static cl::opt<std::string>
    ClMemoryAccessCallbackPrefix("sgxsan-memory-access-callback-prefix",
                                 cl::desc("Prefix for memory access callbacks"),
                                 cl::Hidden, cl::init("__asan_"));

static cl::opt<bool>
    ClInstrumentDynamicAllocas("sgxsan-instrument-dynamic-allocas",
                               cl::desc("instrument dynamic allocas"),
                               cl::Hidden, cl::init(true));

static cl::opt<bool>
    ClSkipPromotableAllocas("sgxsan-skip-promotable-allocas",
...
static cl::opt<int> ClMappingScale("sgxsan-mapping-scale",
                                   cl::desc("scale of asan shadow mapping"),
                                   cl::Hidden, cl::init(0));

static cl::opt<uint64_t>
    ClMappingOffset("sgxsan-mapping-offset",
...
static cl::opt<bool> ClOpt("sgxsan-opt", cl::desc("Optimize instrumentation"),
                           cl::Hidden, cl::init(true));

static cl::opt<bool>
    ClOptSameTemp("sgxsan-opt-same-temp",
                  cl::desc("Instrument the same temp just once"), cl::Hidden,
                  cl::init(true));

static cl::opt<bool> ClOptGlobals("sgxsan-opt-globals",
                                  cl::desc("Don't instrument scalar globals"),
                                  cl::Hidden, cl::init(true));

static cl::opt<bool>
    ClOptStack("sgxsan-opt-stack",
...
static cl::opt<uint32_t>
    ClForceExperiment("sgxsan-force-experiment",
...
static cl::opt<bool>
    ClUseGlobalsGC("sgxsan-globals-live-support",
...
static cl::opt<int> ClDebug("sgxsan-debug", cl::desc("debug"), cl::Hidden,
                            cl::init(0));

static cl::opt<int> ClDebugStack("sgxsan-debug-stack", cl::desc("debug stack"),
                                 cl::Hidden, cl::init(0));

static cl::opt<std::string> ClDebugFunc("sgxsan-debug-func", cl::Hidden,
                                        cl::desc("Debug func"));

static cl::opt<int> ClDebugMin("sgxsan-debug-min", cl::desc("Debug min inst"),
                               cl::Hidden, cl::init(-1));

static cl::opt<int> ClDebugMax("sgxsan-debug-max", cl::desc("Debug max inst"),
                               cl::Hidden, cl::init(-1));
...
void AddressSanitizer::initializeCallbacks(Module &M) {
  ...
  for (int Exp = 0; Exp < 1 /* 2 */; Exp++) {
    for (size_t AccessIsWrite = 0; AccessIsWrite <= 1; AccessIsWrite++) {
      const std::string TypeStr = AccessIsWrite ? "store" : "load";
      const std::string ExpStr = Exp ? "exp_" : "";
      const std::string EndingStr = Recover ? "_noabort" : "";

      SmallVector<Type *, 6> Args4 = {IntptrTy, IntptrTy, Type::getInt1Ty(*C),
                                      Type::getInt8PtrTy(*C)};
...
void AddressSanitizer::declareAdditionalSymbol(Module &M) {
  ...
  // OCAllocStack
  PushOCAllocStack =
      M.getOrInsertFunction("PushOCAllocStack", Type::getVoidTy(*C));
  PopOCAllocStack =
      M.getOrInsertFunction("PopOCAllocStack", Type::getVoidTy(*C));
}
